...
```bash
INVENTORY_TOKEN=$(uv run python inventory/manage.py drf_create_token admin | awk '/Generated token/ {print $3}')
```
...
PATCH treats `deduct_quantity` as a **delta** — the value is subtracted from
current stock (must be at least `1`). Attempting to deduct more than is in
stock returns HTTP 400. Dropping stock below `low_stock_threshold` fires a
Celery task to procurement, which records a restock request.
...
The bearer token is read from the `PROCUREMENT_API_TOKEN` environment variable
(default: `devtoken`, see `.env.example`). Export it into your shell before
calling the API:
...
export PROCUREMENT_API_TOKEN=devtoken

echo "${#PROCUREMENT_API_TOKEN}"   # sanity check — should be > 0
```
